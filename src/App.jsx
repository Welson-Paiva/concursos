    ...
    const initAuth = async () => {
      try {
        ...
        if (session) {
          ...
          setLoading(false); // Para o loop se não houver usuário
        }
      } catch (e) {
        setLoading(false);
      }
...
function Modal({ modal, setModal, user, refreshAll, notify }) {
  // Estados para controlar os inputs sem depender de ID do documento
  const [nomeDisciplina, setNomeDisciplina] = useState(modal.type === 'editA' ? modal.data.nome : "");
  const [nomeAssunto, setNomeAssunto] = useState("");

  const handleSave = async () => {
    try {
      if (modal.type === 'editA') {
        await supabase.from("assuntos").update({ nome: nomeDisciplina }).eq("id", modal.data.id);
      } else if (modal.type === 'full') {
        const { data } = await supabase.from("disciplinas").insert({ user_id: user.id, nome: nomeDisciplina }).select().single();
        if (data) await supabase.from("assuntos").insert({ user_id: user.id, disciplina_id: data.id, nome: nomeAssunto });
      } else if (modal.type === 'assu') {
        await supabase.from("assuntos").insert({ user_id: user.id, disciplina_id: modal.data, nome: nomeAssunto });
      ...
      notify("Erro ao processar dados.", "error");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-[#0a0c10]/98 backdrop-blur-xl flex items-center justify-center p-4">
      <div className="bg-[#0d1117] w-full max-w-sm p-10 rounded-[3rem] border border-white/10 shadow-2xl">
        <h3 className="text-[11px] font-black uppercase text-indigo-500 mb-8 flex items-center gap-2">
          <Zap size={14}/> {modal.type.startsWith('del') ? 'Confirmar Exclusão' : 'Gerenciar Dados'}
        </h3>
        <div className="space-y-5">
            ...
            <>
              {(modal.type === 'full' || modal.type === 'editA') && (
                <input 
                  ...
                  className="w-full bg-[#0a0c10] p-4 rounded-2xl text-white text-xs border border-white/5 outline-none focus:border-indigo-500" 
                />
              )}
          ...
          {modal.type.startsWith('del') && <p className="text-xs text-slate-400 text-center">Deseja remover este item permanentemente?</p>}
          <div className="pt-4 space-y-3">
            <button onClick={handleSave} className="w-full p-5 rounded-2xl bg-indigo-600 text-white font-black text-[10px]">SALVAR</button>
            <button onClick={() => setModal({ type: null })} className="w-full p-5 text-slate-500 text-[10px]">CANCELAR</button>
          </div>
        </div>
      </div>
    </div>
